...
void LoyalityCard::StaticInitialize()
{
    FBLOG_INFO("StaticInit", "Static Initialize");
}

void LoyalityCard::StaticDeinitialize()
{
    FBLOG_INFO("StaticInit", "Static Deinitialize");
}

LoyalityCard::LoyalityCard(const std::string& mimetype) :
    m_mimetype(mimetype)
{
}
...
FB::JSAPIPtr LoyalityCard::createJSAPI()
{
    // m_host is the BrowserHost
    return boost::make_shared<LoyalityCardAPI>(FB::ptr_cast<LoyalityCard>(shared_from_this()), m_host);
}

bool LoyalityCard::onMouseDown(FB::MouseDownEvent *evt, FB::PluginWindow*)
{
...
bool LoyalityCard::onMouseUp(FB::MouseUpEvent *evt, FB::PluginWindow*)
{
    // These are just examples; if you don't need them, remove them, as htmlLog
    // does have a performance penalty!
    std::ostringstream oStr;
...
bool LoyalityCard::onMouseMove(FB::MouseMoveEvent *evt, FB::PluginWindow*)
{
    // These are just examples; if you don't need them, remove them, as htmlLog
    // does have a performance penalty!
    std::ostringstream oStr;
...
bool LoyalityCard::onAttached( FB::AttachedEvent *evt, FB::PluginWindow* win)
{
#ifdef FBWIN_ASYNCSURFACE
...
bool LoyalityCard::onDetached( FB::DetachedEvent *evt, FB::PluginWindow* )
{
    // This is called when the window is detached! You must not draw after this!
    return false;
}

bool LoyalityCard::draw( FB::RefreshEvent *evt, FB::PluginWindow* win )
{
    ...
    if (!m_mimetype.compare("application/x-cardmgr-math")) {
...
bool LoyalityCard::startDrawAsync(FB::D3d10AsyncDrawServicePtr ads)
{
    m_thread = boost::thread(&LoyalityCard::renderThread, this, ads);
    return true;
}
...
void LoyalityCard::renderThread(FB::D3d10AsyncDrawServicePtr ads)
{
    try {
...
void LoyalityCard::onPluginReady()
{
...
uint32_t LoyalityCard::asyncTestBgColor()
{
...
std::string LoyalityCard::negotiateDrawingModel()
{
